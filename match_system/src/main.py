...
class Pool:  # 匹配池
    ...
    def check_match(self, a, b):  # 是否满足 可开局
    ...
    def match(self):
        while len(self.players) >= 3:
            self.players = sorted(self.players, key=lambda p: p.score)  # 玩家排序 尽可能分数相近匹配在一起
            flag = False
            for i in range(len(self.players) - 2):
                a, b, c = self.players[i], self.players[i + 1], self.players[i + 2]
                if self.check_match(a, b) and self.check_match(a, c) and self.check_match(b, c):
                    self.match_success([a, b, c])
                    self.players = self.players[:i] + self.players[i + 3:]  # 将匹配成功的玩家  清除匹配池
                    flag = True
                    break
            if not flag:
                break
